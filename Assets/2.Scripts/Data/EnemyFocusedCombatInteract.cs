...
public class EnemyFocusedCombatInteract : MonoBehaviour
{
    ...
    private void Update()
    {
        ...
        if (Input.GetKeyDown(interactKey))
        {
            ...
            // 던전/필드 공통: 캡슐 기반 컨텍스트 우선 (환경 그대로 전투필드 구성)
            if (playerStats != null && enemyInstance != null && enemyInstance.definition != null)
            {
                var ctx = EnvironmentScanner.Scan(playerStats, p, enemyInstance, grid);
                if (ctx == null)
                    ctx = new EncounterContext(playerStats, enemyInstance, p, e);
    ...
    /// <summary>
    /// 필드↔던전 전환 후에도 현재 활성 GridBoard/Player/Stats를 참조하도록 동기화.
    /// </summary>
